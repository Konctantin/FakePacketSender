...
namespace KopiLua
{
    public partial class Lua
    {
        ...
        public static void LuaMFreeMem<T>(LuaState L, T b)
        {
            LuaMRealloc<T>(L, new T[] { b }, 0);
        }

        public static void LuaMFree<T>(LuaState L, T b)
        {
            LuaMRealloc<T>(L, new T[] { b }, 0);
        }

        public static void LuaMFreeArray<T>(LuaState L, T[] b)
        {
            LuaMReallocV(L, b, 0);
        }

        public static T LuaMMalloc<T>(LuaState L)
        {
            return (T)LuaMRealloc<T>(L);
        }

        public static T LuaMNew<T>(LuaState L)
        {
            return (T)LuaMRealloc<T>(L);
        }
        ...
        */
        ...
        private static void AddTotalBytes(LuaState L, int num_bytes)
        {
            G(L).totalbytes += (uint)num_bytes;
        }

        private static void SubtractTotalBytes(LuaState L, int num_bytes)
        {
            G(L).totalbytes -= (uint)num_bytes;
        }

        private static void AddTotalBytes(LuaState L, uint num_bytes)
        {
            G(L).totalbytes += num_bytes;
        }

        private static void SubtractTotalBytes(LuaState L, uint num_bytes)
        {
            G(L).totalbytes -= num_bytes;
        }
    }
