...
namespace KopiLua
{
    ...
    public partial class Lua
    {
        ...
        private static void callTMres(LuaState L, StkId res, TValue f, TValue p1, TValue p2)
        {
        ...
        private static void callTM(LuaState L, TValue f, TValue p1, TValue p2, TValue p3)
        {
            SetObj2S(L, L.top, f);  /* push function */
            SetObj2S(L, L.top + 1, p1);  /* 1st argument */
            SetObj2S(L, L.top + 2, p2);  /* 2nd argument */
        ...
        private static int call_binTM(LuaState L, TValue p1, TValue p2, StkId res, TMS event_)
        {
        ...
        private static TValue get_compTM(LuaState L, Table mt1, Table mt2, TMS event_)
        {
        ...
        private static int call_orderTM(LuaState L, TValue p1, TValue p2, TMS event_)
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    for (n = 1; n < total && (tostring(L, top - n - 1) != 0); n++)
                    {
                        ...
                        if (l >= MAXSIZET - tl)
                            LuaGRunError(L, "string length overflow");
                        tl += l;
                    }
        ...
        */
        ...
        public static void runtime_check(LuaState L, bool c)
        {
            Debug.Assert(c);
        }
        ...
        internal static TValue RA(LuaState L, StkId base_, Instruction i)
        {
            return base_ + GETARG_A(i);
        }

        internal static TValue RB(LuaState L, StkId base_, Instruction i)
        {
            return base_ + GETARG_B(i);
        }

        internal static TValue RC(LuaState L, StkId base_, Instruction i)
        {
            return base_ + GETARG_C(i);
        }

        internal static TValue RKB(LuaState L, StkId base_, Instruction i, TValue[] k)
        {
            return ISK(GETARG_B(i)) != 0 ? k[INDEXK(GETARG_B(i))] : base_ + GETARG_B(i);
        }

        internal static TValue RKC(LuaState L, StkId base_, Instruction i, TValue[] k)
        {
            return ISK(GETARG_C(i)) != 0 ? k[INDEXK(GETARG_C(i))] : base_ + GETARG_C(i);
        }

        internal static TValue KBx(LuaState L, Instruction i, TValue[] k)
        {
            return k[GETARG_Bx(i)];
        }

        public static void dojump(LuaState L, InstructionPtr pc, int i)
        {
            pc.pc += i; LuaIThreadYield(L);
        }
